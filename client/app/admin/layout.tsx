      ...
      <div className="flex min-h-screen bg-slate-950 text-slate-100">
        ...
        <aside className="hidden lg:flex w-64 flex-col border-r border-white/10 bg-slate-900/90 p-5 space-y-6 shrink-0 backdrop-blur-xl">
          ...
          <div className="flex items-center gap-3 px-2">
            ...
            <div>
              <h2 className="text-base font-extrabold text-white leading-tight">Admin Portal</h2>
              <p className="text-[11px] font-mono text-indigo-400">System Admin</p>
            </div>
          </div>
          ...
          <header className="sticky top-0 z-30 flex items-center justify-between border-b border-white/10 bg-slate-900/80 px-6 py-4 backdrop-blur-xl">
            ...
            <div className="flex items-center gap-3">
              <div className="text-right hidden sm:block">
                <p className="text-sm font-bold text-white">{user?.fullName || 'System Admin'}</p>
                <p className="text-xs text-slate-400">{user?.email || 'admin@school.com'}</p>
              </div>
              <div className="h-9 w-9 rounded-full bg-gradient-to-tr from-indigo-500 to-purple-600 flex items-center justify-center text-white font-bold text-sm shadow-md">
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-semibold ${
                      isActive ? 'bg-indigo-600/20 text-white' : 'text-slate-400 hover:text-white'
                    }`}
                  >
