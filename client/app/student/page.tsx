...
import { GraduationCap, LogOut, UserCheck } from 'lucide-react';
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-6 border-b border-white/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Assignments & Grades Portal</p>
            </div>
          </div>
        ...
        <div className="rounded-2xl border border-white/10 bg-slate-900/60 p-6 sm:p-8 backdrop-blur-xl space-y-4">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-semibold">
            ...
            <span>Authenticated as {user ? getRoleName(user.role) : 'Student'}</span>
          </div>

          <h2 className="text-xl font-bold text-white">Student Dashboard - Coming Soon</h2>
          <p className="text-sm text-slate-400 leading-relaxed">
            Welcome back, <span className="font-semibold text-slate-200">{user?.fullName || 'Student'}</span> ({user?.email})!
            {user?.classId && (
              <span className="block mt-1 text-xs text-indigo-400 font-mono">Assigned Class ID: {user.classId}</span>
            )}
          </p>
        </div>
      </div>
